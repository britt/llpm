...
// import { modelRegistry } /* unused - verified */ from '../services/modelRegistry';
...
export class ProjectDatabase {
  ...
  getNotes(): ProjectNote[] {
    ...
    RequestContext.logDatabaseOperation('select', 'end', {
      table: 'notes',
  ...
  async searchNotesSemantica(
    query: string,
    limit: number = 10
  ): Promise<Array<ProjectNote & { similarity: number }>> {
    return traced(
      'db.searchNotesSemantica',
      {
        attributes: {
          'db.operation': 'search',
          'db.table': 'notes',
  ...
  async searchFilesSemantic(
    query: string,
    limit: number = 10
  ): Promise<Array<ProjectFile & { similarity: number }>> {
