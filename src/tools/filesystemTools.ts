...
async function validateProjectPath(requestedPath: string): Promise<string> {
  ...
  if (!currentProject) {
    throw new Error(
      'No active project set. Use /project switch <project-id> to set an active project.'
    );
  }
  ...
  if (
    ...
    relativePath === '..' ||
    targetPath === projectPath.split(sep)[0]
  ) {
    throw new Error(
      `Access denied: Path '${requestedPath}' is outside the current project directory '${projectPath}'`
    );
  }
...
export const readProjectFile = tool({
  description:
    'Read the contents of a file within the current project directory. Useful for understanding code structure, configuration files, documentation, etc.',
  inputSchema: z.object({
    path: z
      .string()
      .describe('The file path to read (relative to project root or absolute path within project)'),
...
export const listProjectDirectory = tool({
  description:
    'List files and directories within the current project. Useful for exploring project structure and finding relevant files.',
  inputSchema: z.object({
    path: z
      .string()
      .default('.')
      .describe(
        'Directory path to list (relative to project root or absolute path within project)'
      ),
    includeHidden: z
      .boolean()
      .default(false)
...
export const getProjectFileInfo = tool({
  description:
    'Get detailed information about a file or directory within the current project (size, modification time, type, etc.).',
  inputSchema: z.object({
    path: z
      .string()
      .describe(
        'The file or directory path to inspect (relative to project root or absolute path within project)'
      )
  }),
  execute: async ({ path }) => {
    try {
...
export const findProjectFiles = tool({
  description:
    'Search for files within the current project by name pattern. Useful for finding specific files or file types.',
  inputSchema: z.object({
    pattern: z
      .string()
      .describe(
        'File name pattern to search for (supports glob-like patterns: *.js, **/*.ts, etc.)'
      ),
      ...
      const searchDirectory = async (dirPath: string): Promise<void> => {
        ...
        for (const entry of filtered) {
          ...
          if (stats.isFile() && matchesPattern(relativePath, pattern)) {
            const size =
              stats.size > 1024 ? `${Math.round(stats.size / 1024)}KB` : `${stats.size}B`;
