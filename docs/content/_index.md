...
{{< hextra/feature-grid >}}
  {{< hextra/feature-card
    ...
    subtitle="Configure one or more providers, then switch models as your needs change.
...
- Use `/model list` to view available models.
...
- OpenAI: `gpt-5.2`, `gpt-5.2-mini`, `gpt-5.2-turbo`, `gpt-4o-mini`, `o4-mini`
...
If a provider is not configured, its models are hidden from the selector (and the footer only shows models from configured providers)."
    class="hx-aspect-auto md:hx-aspect-[1.1/1] max-md:hx-min-h-[760px]"
...
A project scan summarizes codebase structure, dependencies, and documentation. Use it to scope work with concrete inputs, and avoid re-scanning the same repository every session.
